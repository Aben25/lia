...
export default async function ProtectedPage() {
  ...
  const { data: Sponsors } = await supabase.from("Sponsors").select("*").eq("Email", user.email);
          ...
          Sponsees.map((sponsee) => (
            <div key={sponsee.id} className="bg-white shadow-md rounded-lg p-6 flex">
              ...
              <div className="flex flex-col">
                ...
                <p className="text-sm text-gray-600 mb-4">{sponsee.aspiration ? `Aspires to be a ${sponsee.aspiration}` : "Aspiration not provided"}</p>

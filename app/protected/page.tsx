  ...
  useEffect(() => {
    const fetchStatistics = async () => {
      try {
        ...
        const donationsTrend = donations.reduce<Record<string, number>>((acc, donation) => {
        ...
        const donationsTrendArray = Object.entries(donationsTrend)
          .map(([month, amount]) => {
            ...
            return { 
              month, 
              amount, 
              date: new Date(parseInt(yearStr), new Date(`${monthStr} 1, 2000`).getMonth())
            };
          })
        ...
        const sponsorshipsByCountry = sponsorsByCountry.reduce<Record<string, number>>((acc, sponsor) => {
        ...
        const impactBreakdown = sponseesGrades.reduce<Record<string, number>>((acc, sponsee) => {
